...
base_conf = OmegaConf.load(os.path.join("config", "Unet+submodular_multiclass.yml"))
...
EXPERIMENT_NAME = "Unet(" + conf.params.segmentation_architecture + "+" + conf.params.encoder + ")" + (
    "_SoftClass_apprx" if conf.params.enable_graphcut else "") + "_multiclass"
...
class CNNWithGC(pl.LightningModule):
    ...
    def compute_multiclass_image(self, y, y_norm):
        out = np.array(Image.new(mode="RGB", size=tuple(y.size()[-2:])))
    ...
    def test_step(self, batch, batch_idx):
        ...
        if self.saved_images < conf.images_to_log:
            if conf.log_debug_image and self.use_graph_cut:
                ...
                out_plot = axarr[1][2].imshow(self.last_pre_cut_image[0, 1, ...].detach().cpu())
                axarr[1][2].axis('off')
                axarr[1][2].set_title("Pre cut output (class 0)")
                fig.colorbar(out_plot, ax=axarr[1][2])
                out_plot = axarr[1][3].imshow(self.last_pre_cut_image[0, 2, ...].detach().cpu())
                axarr[1][3].axis('off')
                ...
                axarr[2][2].imshow(transforms.ToPILImage()(y[1].detach().cpu()).convert('1', dither=Image.NONE))
                axarr[2][2].axis('off')
                axarr[2][2].set_title("Ground Truth (class 0)")
                axarr[2][3].imshow(transforms.ToPILImage()(y[2].detach().cpu()).convert('1', dither=Image.NONE))
                axarr[2][3].axis('off')
