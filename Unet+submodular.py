...
base_conf = OmegaConf.load(os.path.join("config", "Unet+submodular.yml"))
...
EXPERIMENT_NAME = "Unet(" + conf.params.segmentation_architecture + "+" + conf.params.encoder + ")" + (
    "_GC" if conf.params.enable_graphcut else "")
...
# Files to upload on Neptune.ai
FILES_TO_UPLOAD = [str(__file__), "segmentation_dataset.py", "datasets_dic.py", "utils.py"]
...
class CNNWithGC(pl.LightningModule):
    ...
    def forward(self, x):
        ...
        if self.use_graph_cut:
            ...
            pixel_scores = self.gc(pixel_scores, weights_row, weights_col)
            if self.skip_connection:
...
neptune_logger = NeptuneLogger(params=dict(conf.params),
                               ...
                               upload_source_files=FILES_TO_UPLOAD,
                               #offline_mode=True,
