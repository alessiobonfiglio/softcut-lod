...
from softcut.SoftCut_exact import SoftGraphCut
...
if __name__ == '__main__':
    ...
    base_conf = OmegaConf.load(os.path.join("config", "Unet+SoftCut_exact.yml"))
    ...
    EXPERIMENT_NAME = "Unet(" + conf.params.segmentation_architecture + "+" + conf.params.encoder + ")" + (
        "_SoftCut_exact" if conf.params.enable_graphcut else "")
    ...
    train_loader = DataLoader(train_ds, batch_size=conf.params.batch_size, num_workers=conf.num_of_workers,
                            shuffle=conf.params.shuffle_training_set, pin_memory=True)
    val_loader = DataLoader(val_ds, batch_size=conf.params.batch_size, num_workers=conf.num_of_workers, pin_memory=True)
    test_loader = DataLoader(test_ds, batch_size=1, num_workers=conf.num_of_workers, pin_memory=True)
    ...
    class CNNWithGC(pl.LightningModule):
        def __init__(self, lr, batch_size,
            ...
            if self.use_graph_cut:
                self.gc = SoftGraphCut((256, 256), num_workers=num_workers, multiprocess=use_multithreading_for_graphcut)
        ...
        def forward(self, x):
            ...
            return pixel_scores.contiguous()

        def compute_accuracy_on_positive_class(self, y_hat, y):
        ...
        def extra_test_metrics(self, y_hat, y, suffix=""):
            from sklearn import metrics
            import numpy
            import warnings

            n_elem = torch.numel(y)
            y_hat_tresholded = y_hat > self.accuracy_threshold
            y = y.detach().cpu().numpy().flatten()
            y_hat = y_hat.detach().cpu().numpy().flatten()
            y_hat_tresholded = y_hat_tresholded.detach().cpu().numpy().flatten()
            tn, fp, fn, tp = metrics.confusion_matrix(y, y_hat_tresholded, labels=[0., 1.]).ravel()
            tn_norm = tn / n_elem
            fp_norm = fp / n_elem
            fn_norm = fn / n_elem
            tp_norm = tp / n_elem
            with warnings.catch_warnings():
                ...
                matthews_corr = numpy.nan_to_num((tp * tn - fp * fn) / numpy.sqrt((tp + fp) * (tp + fn) * (tn + fp) * (tn + fn)),
                                                nan=numpy.nan, posinf=numpy.nan, neginf=numpy.nan)
                pr_auc = numpy.nan_to_num(metrics.average_precision_score(y, y_hat, pos_label=1.), nan=numpy.nan,
                                        posinf=numpy.nan, neginf=numpy.nan)
            if len(numpy.unique(y)) > 1:
                roc_auc = metrics.roc_auc_score(y, y_hat, labels=[0., 1.])
            else:
                roc_auc = numpy.nan
            cross_entropy = metrics.log_loss(y, y_hat, eps=1e-7, labels=[0., 1.])
