...
EXPERIMENT_NAME = "submodular_nips17_CNN+submodular"
...
class CNNWithGC(pl.LightningModule):
    ...
    def __init__(self, lr, batch_size, use_isotonic_regression, num_workers,
                 use_multithreading_for_graphcut):
        super().__init__()
        self.cnn_pixel_score = nn.Sequential(nn.Conv2d(3, 32, 3, padding=1),
                                             nn.ReLU(),
                                             nn.Conv2d(32, 64, 3, padding=1),
                                             nn.ReLU(),
                                             nn.Conv2d(64, 1, 3, padding=1))
        self.cnn_weights_row = nn.Sequential(nn.Conv2d(3, 32, 3, padding=1),
                                             nn.ReLU(),
                                             nn.Conv2d(32, 1, 3, padding=1))
        self.cnn_weights_col = nn.Sequential(nn.Conv2d(3, 32, 3, padding=1),
                                             nn.ReLU(),
                                             nn.Conv2d(32, 1, 3, padding=1))
        self.use_isotonic_regression = use_isotonic_regression
        self.num_workers = num_workers
        self.use_multithreading_for_graphcut = use_multithreading_for_graphcut
    ...
    def forward(self, x):
        ...
        pixel_scores = self.cnn_pixel_score(x)

        weights_row = self.cnn_weights_row(x)
        weights_col = self.cnn_weights_col(x)

        # we remove the unneeded rows/columns
        weights_row = weights_row[:, :, :, :-1]
        weights_col = weights_col[:, :, :-1, :]

        # we remove (but re-add it later) the channel dimension
        ...
        weights_row = torch.exp(weights_row)
        weights_col = torch.exp(weights_col)

        segmentation = self.gc(pixel_scores, weights_row, weights_col)
        return torch.unsqueeze(segmentation, 1)
    ...
    def training_step(self, batch, batch_idx):
        ...
        accuracy = CNNWithGC.compute_accuracy_with_logits(y_hat, y)
...
model = CNNWithGC(conf.params.learning_rate, conf.params.batch_size,
                  conf.params.refine_with_isotonic_regression,
                  conf.num_of_workers,
                  conf.params.use_multiple_threads_for_graph_cut)
